...
class G05A_PointsAndLines(SafeScene):
    def construct(self):
        ...
        # Q ค้างอยู่บนจอทั้งซีน มีเส้นที่ออกจากตัว Q เองพาดผ่านหลายทิศ (ไป A, ไป B,
        # เส้น normal ไป P, ลูกศรไป E, ลูกศรไป F) -- ลอง UL แล้วเจอชน (เจอจริงจาก
        # [LAYOUT] log รอบแรก) ลอง UP ต่อก็ยังชน (รอบสอง ชนเส้นไป A ซึ่งอยู่แค่
        # ~75° จาก UP) แก้แบบเดียวกับที่ G06 ใช้กับจุด R/P: หาช่องว่างเชิงมุมกว้าง
        # ที่สุดจริง -- ช่องว่างที่กว้างสุดคือระหว่างทิศไป A กับทิศไป B (เกือบตรงข้าม
        # กันอยู่แล้ว เพราะ A,B คนละฝั่งเส้นศูนย์กลางที่ผ่าน Q) ใช้ทิศตรงข้ามกับ
        # ผลรวมของทั้งสองทิศนั้น (ออกจากทั้งคู่พร้อมกัน)
        _qa = normalize(A - Q)
        _qb = normalize(B - Q)
        _tQdir = normalize(-(_qa + _qb))
        dQ, tQ = pt(Q, WHITE, 0.09), tag("Q", Q, _tQdir, WHITE, 26, 0.16)
        ...
        # ก็ฉายตั้งฉากลงเส้น normal ที่จุด P เดียวกันโดยนิยาม) เพราะงั้นทิศตั้งฉาก
        # กับ (E-P) จะขนานกับ U (เส้น normal) เสมอด้วย -- ลองแล้วเจอชน n_line จริง
        # (เจอจริงจาก [LAYOUT] log รอบสอง: 'E' ทับ Line) ต้องเลี่ยงทั้งเส้นประ P-E-F
        # (ทิศ normalize(E-P)) และลูกศร a_v2 (Q->E, มองจาก E ย้อนไปหา Q) พร้อมกัน
        # -- ใช้ทิศกึ่งกลางระหว่างสองเส้นนี้ (ห่างจาก n_line เองโดยธรรมชาติอยู่แล้ว
        # ~148° เพราะ E-P ตั้งฉากกับ U อยู่แล้ว)
        _e1 = normalize(E - P)
        _e2 = normalize(Q - E)
        _tEdir = normalize(_e1 + _e2)
        dE, tE = pt(E, C_VQ2, 0.075), tag("E", E, _tEdir, C_VQ2, 24, 0.16)
        ...
        # ปัญหาโครงสร้างเดียวกับ E ข้างบน (F-P ก็ตั้งฉากกับ U เสมอด้วยเหตุผล
        # เดียวกัน -- ทิศตั้งฉากกับ F-P จึงขนาน n_line เหมือนกัน) เลี่ยงด้วยวิธี
        # เดียวกับที่ Q ใช้ข้างบน: หาทิศตรงข้ามกับผลรวมของสองเส้นที่โผล่จาก F เอง
        # (เส้นประไป P/ผ่าน E, ลูกศร a_v3 ไป Q) แม้รอบนี้ยังไม่เคยถูก [LAYOUT] จับ
        # ได้ตรงๆ (F ไกลจาก n_line กว่า E เกือบ 3 เท่า) แต่ความเสี่ยงเชิงโครงสร้าง
        # เหมือนกันทุกประการ แก้เชิงรุกไปพร้อมกันดีกว่าปล่อยไว้
        _f1 = normalize(P - F)
        _f2 = normalize(Q - F)
        _tFdir = normalize(-(_f1 + _f2))
        dF, tF = pt(F, C_VQ3, 0.075), tag("F", F, _tFdir, C_VQ3, 24, 0.16)
