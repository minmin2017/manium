...
class G05A_PointsAndLines(SafeScene):
    def construct(self):
        ...
        # Q ค้างอยู่บนจอทั้งซีน มีเส้น/โค้งบรรจบถึง 8 ทิศ: ไป A, ไป B, เส้น normal
        # 2 ทาง (+U/-U), ลูกศรไป E, ลูกศรไป F, และ -- ที่พลาดไปหลายรอบ -- arc2/arc3
        # (ผิวโค้งชิ้นงาน) ที่สัมผัสกันพอดีที่ Q จึงมี "เส้นสัมผัสร่วม" (common
        # tangent) ที่ Q ตั้งฉากกับ U ทั้งสองทาง (เจอจริงจากเฟรมจริงรอบก่อน: 'Q'
        # ทับ Arc ที่ arc2/arc3 ยังไม่ถูก FadeOut ตอนนั้น) ใช้ gap_dir() คำนวณจาก
        ...
        # รอบสอง (Gemini frame review 2026-09-05, หลัง fix R/P ใน G06): เส้น
        # "line of centers" (loc, A-B) ตัดผ่านตัว "v" ของป้ายนี้เอง -- ตรวจแล้ว
        # ว่า gap_dir() แบบเดิมไม่ช่วย แม้เพิ่มจุดที่ใกล้ loc ที่สุดเข้าไปในลิสต์
        # หลีกเลี่ยงด้วย (ทิศที่ได้แทบไม่ขยับ เพราะช่องว่างเชิงมุมกว้างสุดยังอยู่
        # แถวเดิม ~45° ซึ่งพา bbox ของข้อความไปเกยแนวตั้งของ loc พอดี เนื่องจาก
        # loc เป็นเส้นตรงยาว ไม่ใช่จุดเดียว การเลี่ยง "จุดที่ใกล้ที่สุด" อย่างเดียว
        # ไม่พอ) แก้ครั้งแรกเป็นทิศ UR (45°) ตรวจว่า "ผ่าน" ตอนนั้นเพราะเช็คแค่จุด
        # dense_points ของ DashedLine เอง (ไม่ใช่เส้นทึบ) ที่ dash phase หนึ่งพอดี --
        # รอบสาม (Gemini ยัง flag ที่จุดเดิมซ้ำหลัง fix อื่นๆ, ยืนยันจริงด้วยการ
        # ซูมภาพเรนเดอร์จริงแบบ nearest-neighbor สูงสุด: dash จริงทะลุตัว "v" จริง)
        # สาเหตุแท้จริง: a_v2.get_center() (จุดยึดตั้งต้น) อยู่ห่างจาก x ของ loc
        # แค่ 0.134 หน่วย ซึ่งน้อยกว่าครึ่งความกว้างของป้าย (~0.186 หน่วย) เอง --
        # ไม่ว่าจะหมุนทิศไปทางไหนที่ buff แคบ (0.10 เดิม) bbox ของข้อความก็คร่อม
        # แนวตั้งของ loc อยู่ดี แค่ "บังเอิญไม่ชนจุดที่สุ่มมา" ในบาง phase ของ dash
        # เท่านั้น -- ไม่ใช่การเคลียร์จริง (ตรวจซ้ำด้วยการจำลอง loc เป็นเส้นทึบ 100%
        # ไม่มีช่องว่างเลย: ทุกทิศที่ buff=0.10 มี margin ~0) ทางแก้จริงต้องเพิ่ม
        # buff ให้มากพอที่ bbox ทั้งก้อนอยู่ "คนละฝั่ง" ของเส้น loc ไปเลย ไม่ใช่แค่
        # เลี่ยงจุดบนเส้น -- สแกน 360° ที่ buff=0.30 เจอทิศ 26° ให้ bbox เคลียร์
        # เส้น loc ทั้งแบบจริง (มีช่องว่าง) และแบบจำลองเป็นเส้นทึบ (margin ~0.17
        # หน่วยในกรณีเลวร้ายสุด) พร้อมเคลียร์ l_AQ, n_line, a_v2, ra2, dQ, dE, และ
        # ป้าย Q/AQ ข้างเคียงทั้งหมดด้วย
        t_v2 = MathTex("v_{Q_2}", font_size=26, color=C_VQ2).next_to(
            a_v2.get_center(), np.array([np.cos(26 * DEGREES), np.sin(26 * DEGREES), 0.0]), buff=0.30)
        ...
        # ลูกศรทั้งสองเพราะไม่ได้ผ่าน P ตรงๆ) -- รอบสาม (Gemini frame review
        # 2026-09-05, หลัง fix v_Q2/loc ด้านบน): เส้น BQ (l_BQ, สีส้ม) พาดผ่านขา
        # ซ้ายของตัวอักษร "P" -- ตรวจด้วยเลขจริงว่า gap_dir() แบบเดิม (ไม่นับ BQ)
        # ห่างจาก BQ แค่ ~0.03-0.05 หน่วยโลก (ไม่ถึง 0.005 ของ OVERLAP_MIN_AREA
        # linter เลยไม่เตือน แต่ใกล้พอที่ font metric ต่างกันระหว่างเครื่องนี้กับ
        # cloud container จะดันให้ทับจริงได้) เพิ่มจุดใกล้ BQ เข้า gap_dir() แล้ว
        # ทิศใหม่ที่ได้ (เกือบ RIGHT) ดันไปทับจุด dE แทน (ยังใกล้กลุ่ม E/F/Q อยู่ดี
        # เพราะกลุ่มนี้ล้อมรอบ P เกือบทุกทิศ) แก้ด้วยการสแกนมุมทั้ง 360° จริง
        ...
        # ด้วย MANIM_THAI_FONT=Loma จริง -- **บทเรียนที่สอง**: การเช็คตอนนั้นยังนับ
        # pr_E/pr_F เป็น DashedLine (มีช่องว่างจริง) เหมือนกับที่ v_Q2 เจอปัญหา
        # เดียวกันกับ loc (ดู comment ด้านบน) -- แม้ครั้งนี้ [LAYOUT] cloud ผ่านจริง
        # (ไม่เหมือน v_Q2 ที่ Gemini ยังจับซ้ำได้) แต่เพื่อความชัวร์ (n_line เป็นเส้น
        # ทึบจริงและ margin เดิมแค่ ~0.03-0.05 ที่ 284°/buff=0.25) จึงตรวจซ้ำโดย
        # จำลอง pr_E/pr_F/loc เป็นเส้นทึบ 100% ทั้งหมด แล้วสแกน 360° ใหม่อีกรอบ --
        # ทิศ RIGHT ตรงๆ ที่ buff=0.35 ให้ margin ~0.10 หน่วยกับทุกอย่าง (l_BQ, l_AQ,
        # n_line, a_v2, a_v3, dE, dF, dQ, loc, pr_E, pr_F, l_QP ทั้งแบบจริงและแบบ
        # เส้นทึบ) จุดนี้แน่นเพราะ E/F/Q/BQ/AQ/normal ล้อมรอบ P เกือบทุกทิศจริงๆ
        # (ธรรมชาติของจุดที่มีเส้นโครงหลายเส้นมาบรรจบ) จึง margin สูงสุดที่หาได้จริง
        # อยู่แค่ ~0.10-0.13 หน่วยไม่ว่าจะเลือกทิศ/buff เท่าไหร่ -- **บทเรียนที่สาม**:
        # การเช็ครอบ RIGHT/buff=0.35 ลืมป้ายข้อความ "E" (tE) ไปเลย เช็คแค่จุด dE
        # (Dot) ไม่ได้เช็ค tag("E",...) ที่เป็นตัวอักษรจริง -- cloud render จริง
        # (2026-09-05 รอบถัดมา) เจอ 'P' ทับ Line 25 จุด **และ** 'E' ทับ 'P' ตัวอักษร
        # 54% เพราะทิศ RIGHT พา P ตรงเข้าใกล้ E ที่มีป้ายกำกับอยู่แล้วพอดี แก้โดยรวม
        # เช็ค tE, tF, tQ, t_AQ (ป้ายข้อความจริงทุกป้ายที่ยังอยู่บนจอตอนนี้ ไม่ใช่
        # แค่จุด Dot) เข้าไปในสแกน 360° รอบใหม่ด้วย ได้ทิศ 284° buff=0.5 (ไกลจากจุด
        # P มากกว่าเดิมพอสมควร แต่ยังอยู่ในกรอบเฟรมสบายๆ และห่างจากป้าย E ~0.76
        # หน่วย) ให้ margin ~0.15 หน่วยกับทุกอย่างจริง (กราฟิกทั้งเส้นทึบ+เส้นประ
        # และป้ายข้อความทุกป้าย) -- บทเรียนสะสม 3 รอบนี้: การเช็ค "ทุกอย่างที่อยู่
        # บนจอ" ต้องรวมทั้งกราฟิก (จำลองเป็นเส้นทึบเสมอถ้าต้นฉบับเป็นเส้นประ) และ
        # ข้อความอื่นทุกป้าย (ไม่ใช่แค่จุด Dot ของมัน) มิฉะนั้นจะพลาดได้ง่ายมาก
        _P_DIR = np.array([np.cos(284 * DEGREES), np.sin(284 * DEGREES), 0.0])
        dP, tP = pt(P, C_VN, 0.10), tag("P", P, _P_DIR, C_VN, 26, 0.5)
...
class G06_PitchPoint(SafeScene):
    def construct(self):
        ...
        # เจอจริงจาก [LAYOUT] log 2026-09-05 (re-render หลัง fix RecursionError):
        # 'R' <-> 'P' ทับกัน 42% -- g1 (A-R-P, ศูนย์กลาง y=1.45) กับ g2 (B-S-P,
        # ศูนย์กลาง y=-0.35) เป็นสามเหลี่ยมทรงเดียวกัน (มุม/สัดส่วนเท่ากันทุกจุด
        # เพราะข้อมูลจริงสมมาตรฝั่ง A กับฝั่ง B) วางซ้อนกันแนวตั้งห่างกันแค่ 1.8
        # หน่วย -- upright_tri() ชี้ป้ายแต่ละมุม "ออกจาก centroid ของสามเหลี่ยม
        # ตัวเอง" เสมอ ซึ่งบังเอิญทำให้มุมขวาบนสุดของ "แต่ละ" สามเหลี่ยม (R ของ g1,
        # P ของ g2) ชี้ป้ายเข้าหาช่องว่างระหว่างสองสามเหลี่ยมพร้อมกันทั้งคู่ (R ชี้
        # ลง-ขวา จาก centroid ที่อยู่เหนือมัน, P ชี้ขึ้น-ขวา จาก centroid ที่อยู่ใต้
        # มัน) จึงไปเจอกันกลางช่องว่าง -- ตรวจด้วยเลขจริงแล้วว่า g1 ไม่มีที่ขยับขึ้น
        # อีก (ป้าย P ของมันเองเกือบชน caption_top อยู่แล้วที่ y~2.5) จึงแก้ที่ g2
        # อย่างเดียว: ดันป้าย "P" ของ g2 ให้ชี้ RIGHT ตรงๆ (แทนทิศทแยงขึ้น-ขวาเดิม)
        # ผลคือป้ายอยู่ที่ y~0.11-0.34 (เกาะระดับตัวจุด P พอดี) ห่างจาก g1's "R"
        # (y~0.44-0.67) อย่างน้อย 0.1 หน่วย -- ตรวจแล้วไม่ชนเส้นขอบสามเหลี่ยม g2
        # เอง (buff 0.30 เท่าเดิม) และไม่ชนป้าย "S" ของ g2 (อยู่คนละย่าน y)
        g2[4][2].next_to(v2u["c"], RIGHT, buff=0.30)
