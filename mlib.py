...
# Leelawadee UI is Windows-only. Cloud render (GitHub Actions, Ubuntu) has no
# such font installed — set MANIM_THAI_FONT (e.g. "Loma" from fonts-thai-tlwg)
# in that environment. Local Windows runs are unaffected (env var unset).
...
# เดิม -3.45 (ชิดขอบล่างสุดเกินไป) — Min แจ้งว่าแถบควบคุมของโปรแกรมเล่นวิดีโอ
# (Windows Movies&TV เป็นต้น) บังคำบรรยายทับ ขยับขึ้นมาให้มีระยะห่างจากขอบจริง
CAP_Y = -3.15                # แถบคำบรรยายล่าง
