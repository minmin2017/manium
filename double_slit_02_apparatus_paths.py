...
Scenes:
  - DS03_ApparatusCoherence (Target: 60-85s, Planned: ~68s)
  - DS04_PointPInvestigation (Target: 90-120s, Planned: ~105s)
"""
...
class DS03_ApparatusCoherence(SafeThreeDScene):
    ...
    def construct(self):
        ...
        # 1. Title & Initial Overview (Micro-beat 1: ~15s)
        ...
        # 2. Name Each Component (Micro-beat 2: ~13s)
        self.play(FadeIn(apparatus_grp), run_time=2.2)
        self.wait(4.0)

        # 3. Incident Plane Waves (Micro-beat 3: ~16s)
        ...
        self.play(Create(pw_hit), run_time=1.2)
        self.wait(3.5)

        # 4. Synchronized Secondary Wavefronts & Locked Phase Clocks (Micro-beat 4: ~14s)
        ...
        # 5. Transition to Top View (Micro-beat 5: ~10s)
...
class DS04_PointPInvestigation(SafeScene):
    """
    Beat 4: Investigating Point P on Screen (~105s)
    Locked Top View (first 60s without insets).
    Micro-beats:
      1. Top-down geometry establish, point P0 at center (16s)
      2. Wave packets travel to P0, same frame arrival, bright flash (20s)
      3. Move P to first minimum, compute r1 and r2, Delta r = lambda/2, dark sum (22s)
      4. Live path ruler dashboard, phase dial (22s)
      5. Sweep P from -2 to +2, screen inset, accumulate cos^2 intensity profile (25s)
    """
    def construct(self):
        ...
        # 1. Top View Setup & Center Point P0 (Micro-beat 1: ~16s)
        b_x = -3.5
        scr_x = 2.5
        d_val = 2.0
        ...
        self.play(FadeIn(dot_p), FadeIn(lbl_p), Create(ray1_p0), Create(ray2_p0), run_time=1.8)
        self.wait(3.5)

        # 2. Center Arrival & Bright Flash (Micro-beat 2: ~20s)
        ...
        self.play(FadeIn(flash_p0), ReplacementTransform(lbl_p, lbl_bright), run_time=1.0)
        self.wait(4.5)

        # 3. Move P to First Minimum (Micro-beat 3: ~22s)
        ...
        self.play(Create(extra_seg), FadeIn(lbl_delta), run_time=1.5)
        ...
        self.play(FadeIn(dark_marker), FadeIn(lbl_dark), run_time=1.0)
        self.wait(4.5)

        # 4. Live Dashboard Introduction (Micro-beat 4: ~22s)
        cap3 = caption_top("แดชบอร์ดแสดงผลต่างทางเดิน: เมื่อเลื่อน P ค่า Delta r จะเปลี่ยนต่อเนื่อง ส่งผลให้ความสว่างเปลี่ยนเป็นคลื่น")
        self.play(
        ...
        # 5. Front-Screen Inset & Continuous Sweep Accumulation (Micro-beat 5: ~25s)
