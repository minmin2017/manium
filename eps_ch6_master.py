...
class S2_NeutralPlane(SafeThreeDScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(cap1), FadeIn(cap2), FadeIn(coil), FadeIn(emf_row),
                  run_time=0.9)
        ...
        cap3 = self.hud(caption_top("จุดบน-ล่างคือที่ emf = 0 → ลากเป็นเส้น = ระนาบเป็นกลาง",
                                    color=OK))
        self.play(FadeOut(cap2), FadeIn(cap3), Create(npl), FadeIn(np_lab),
                  run_time=1.2)
        self.wait(1.0)
        ...
        self.play(FadeOut(cap3), FadeIn(cap4), FadeIn(comm), FadeIn(br), run_time=1.0)
        self.wait(1.3)
...
class S3_TwoFields(SafeThreeDScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(cap1), FadeIn(cap2), FadeOut(ref), FadeIn(ref2),
        ...
        self.play(FadeOut(cap3), FadeIn(cap4), FadeOut(ref2), FadeIn(ref3),
    ...
    def construct(self):
        ...
        for i, nm in enumerate(["A", "B", "C"]):
            ...
            clabels.add(Text(nm, font_size=22, color=CURRENT)
                        .move_to([x, bar_y + bar_h / 2 + 1.08, 0]))
        ...
        self.play(FadeOut(d0), FadeIn(d1), FadeIn(bars), FadeIn(blabels),
        ...
        self.play(FadeOut(d1), FadeIn(d2), FadeIn(i1), FadeIn(i2), run_time=1.0)
        self.wait(1.2)
        ...
        self.play(FadeOut(d2), FadeIn(d3), Create(hl), FadeIn(bnote), run_time=1.1)
        self.wait(1.4)

        # ---------- ส่งไม้ผลัด ----------
        d4 = caption_top("แปรงถ่านเลื่อน → ซี่ 1 รับเพิ่ม 50→100 A · ซี่ 2 ปล่อย 50→0 A")
        self.play(FadeOut(d3), FadeIn(d4), run_time=0.6)
...
class S6_SelfInduction(SafeScene):
    ...
    def construct(self):
        ...
        clab = Text("ขด A", font_size=22, color=CURRENT).move_to([cx, 1.35, 0])
        ...
        self.play(FadeOut(d1), FadeIn(d2), run_time=0.6)
        ...
        self.play(FadeOut(d2), FadeIn(d3), FadeOut(rlab), run_time=0.6)
...
class S7_Interpole(SafeThreeDScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(cap1), FadeIn(cap2), Create(wire), run_time=1.2)
        self.wait(1.0)
...
class S8_Compensating(SafeScene):
    ...
    def construct(self):
        ...
        self.play(FadeOut(cap0), FadeIn(cap1),
