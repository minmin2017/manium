...
def _ic04_banner(text, color=COL_OK):
    bg = RoundedRectangle(
        width=11.8, height=0.52, corner_radius=0.1,
        color=color, fill_color=COL_BG_BOX
    ).set_fill(COL_BG_BOX, 0.95).move_to([0.0, -2.90, 0.0])
...
class IC04_NotationI12(SafeScene):
    """
    W03 น.9 — สัญกรณ์ IC สัมพัทธ์ระหว่างวัตถุสองชิ้น (I12/I21)
    Plan: Main_note/Claude_Specs/Manim — IC04_NotationI12 Plan.md
    """
    ...
    def construct(self):
        ...
        # BEAT 0.0–1.8: Title & Page Reference
        # ======================================================================
        self.title_m = _ic04_title("สัญกรณ์ IC สัมพัทธ์: I₁₂")
        self.ref_m = _ic04_page_ref("W03 น.9")
        self.play(FadeIn(self.title_m, shift=UP * 0.4), FadeIn(self.ref_m), run_time=1.2)
        self.wait(0.4)

        # ======================================================================
        # BEAT 1.8–4.6: Hook Question
        # ======================================================================
        ...
        # ======================================================================
        # BEAT 4.6–15.0: Relative IC between two bodies (link1 fixed, link2 free)
        # ======================================================================
        cap1 = _ic04_caption_top(
            "จริงๆ มี IC สัมพัทธ์ระหว่างวัตถุทุกคู่ในกลไก สัญกรณ์ I₁₂ (หรือ I₂₁ — สลับเลขได้ ความหมายเดียวกัน)"
        )
        fit_width(cap1, 11.8)
        self.play(FadeIn(cap1, shift=UP * 0.35), run_time=0.5)

        # Link 1: fixed frame (gray horizontal bar + ground hatching underneath)
        f_y = -1.35
        link1_bar = Line(start=[-4.6, f_y, 0], end=[-1.4, f_y, 0], color=COL_METAL, stroke_width=7)
        link1_hatches = VGroup(*[
            Line(start=[x, f_y, 0], end=[x - 0.18, f_y - 0.22, 0], color="#64748B", stroke_width=1.5)
            for x in np.linspace(-4.5, -1.6, 10)
        ])
        lbl_link1 = Text("ชิ้นส่วน 1 (Fixed / Frame)", font_size=10, color=COL_METAL).next_to(link1_bar, DOWN, buff=0.55)
        link1_fixed = VGroup(link1_bar, link1_hatches, lbl_link1)

        # Pin A0 sitting on link 1
        pt_A0 = np.array([-3.0, f_y, 0.0])
        pin_A0 = Dot(pt_A0, radius=0.09, color=WHITE)
        label_A0 = Text("A₀", font_size=12, color=WHITE, weight=BOLD).next_to(pin_A0, RIGHT, buff=0.12)

        # Link 2: free-moving rigid body pinned to A0, initial angle
        ang2 = np.radians(70)
        len2 = 2.3
        tip2 = pt_A0 + np.array([len2 * np.cos(ang2), len2 * np.sin(ang2), 0])
        link2_bar = Line(start=pt_A0, end=tip2, stroke_width=8, color=COL_FIELD)
        link2_head = Dot(tip2, radius=0.10, color=COL_FIELD)
        lbl_link2 = Text("ชิ้นส่วน 2 (เคลื่อนที่ได้)", font_size=10, color=COL_FIELD).next_to(tip2, UP, buff=0.18)
        link2_body = VGroup(link2_bar, link2_head, lbl_link2)

        self.play(Create(link1_fixed), run_time=0.5)
        self.play(FadeIn(pin_A0), FadeIn(label_A0), run_time=0.3)
        self.play(FadeIn(link2_body, shift=UP * 0.15), run_time=0.5)
        self.wait(0.3)

        # Rotate link 2 slightly about A0 to show it's genuinely moving
        self.play(Rotate(link2_body, angle=0.3, about_point=pt_A0), run_time=1.5)
        self.wait(0.3)

        # Highlight pin A0, then relabel it as the relative IC I12
        self.play(Indicate(pin_A0, color=COL_OK), run_time=0.7)
        label_I12 = Text("I₁₂", font_size=14, color=COL_OK, weight=BOLD).next_to(pin_A0, RIGHT, buff=0.12)
        self.play(TransformFromCopy(label_A0, label_I12), run_time=0.6)
        ring_A0 = Circle(radius=0.24, color=COL_OK, stroke_width=2.5).move_to(pt_A0)
        self.play(Create(ring_A0), run_time=0.4)

        explain = Text(
            "จุด A₀ (หมุดยึด) มีความเร็วเป็นศูนย์เท่ากันทั้ง 2 ชิ้น เพราะชิ้น 1 หยุดนิ่งอยู่แล้ว → I₁₂ = A₀",
            font_size=12, color=COL_OK
        ).move_to([0.0, -2.45, 0.0])
        fit_width(explain, 11.6)
        ...
        # ======================================================================
        sum_box = RoundedRectangle(
            width=11.2, height=1.6, corner_radius=0.15,
            color=COL_OK, fill_color=COL_BG_BOX
        ).set_fill(COL_BG_BOX, 0.95).move_to([0.0, -0.15, 0.0])
        sum_text = Text(
            "I₁₂ = I₂₁ = จุดที่ความเร็วสัมพัทธ์ระหว่างวัตถุ 2 ชิ้นเป็นศูนย์ (สลับเลขได้ ความหมายเดียวกัน)",
            font_size=13, color=WHITE
        ).move_to([0.0, -0.15, 0.0])
        fit_width(sum_text, 10.6)
        summary_card = VGroup(sum_box, sum_text)
        ...
        # ======================================================================
        # BEAT 17.0–19.3: Review Question (Teaser for IC05)
        # ======================================================================
        q_box = RoundedRectangle(
            width=11.4, height=2.2, corner_radius=0.15,
            color=COL_WARN, fill_color=COL_BG_BOX
        ).set_fill(COL_BG_BOX, 0.95).move_to([0.0, -0.15, 0.0])
        q_body = Text(
            "ถ้าวัตถุ 2 ชิ้นไม่ได้ต่อกันด้วยหมุดตรงๆ (เช่น เลื่อนกัน หรือเป็นเฟือง) จะหา I₁₂ ยังไง?",
            font_size=13, color=WHITE
        ).move_to([0.0, 0.15, 0.0])
        fit_width(q_body, 10.6)
        q_ans = Text(
            "(คลิปต่อไปจะเฉลยทีละกรณี)",
            font_size=11, color=COL_GRAY
        ).move_to([0.0, -0.55, 0.0])
        question_grp = VGroup(q_box, q_body, q_ans)
